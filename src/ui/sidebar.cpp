...
{
    QStringList menuItems = { "Help and Credits", "Vote", "My Votes", "Create Poll", "My Polls", "Profile" };
	for (const QString& item : menuItems)
	{
		...
		connect(btn, &QPushButton::clicked, [item]()
			{
                if (item == "Help and Credits") NavigationManager::instance().navigate(NavigationManager::Help);
